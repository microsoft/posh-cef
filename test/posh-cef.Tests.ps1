...
$Collection += New-Object -TypeName PSObject -Property ([ordered]@{'DeviceVendor'='Contoso';'DeviceProduct'='MyPowershellScript';'DeviceVersion'='1.0';'DeviceEventClassId'='Alert1';'Name'='Foo was detected.';'Severity'='9';'act'='Blocked'})
$Collection += New-Object -TypeName PSObject -Property ([ordered]@{'DeviceVendor'='Contoso';'DeviceProduct'='MyPowershellScript';'DeviceVersion'='1.0';'DeviceEventClassId'='Alert2';'Name'='Bar was detected.';'Severity'='10';'deviceAction'='Blocked'})

$Collection | FT
$Collection | New-CEFMessage | Write-Host -ForegroundColor Cyan
$Collection | %{New-CEFMessage -DeviceVendor $_.DeviceVendor -DeviceProduct $_.DeviceProduct -DeviceVersion $_.DeviceVersion -DeviceEventClassId $_.DeviceEventClassId -Name $_.Name -Severity $_.Severity} | Write-Host -ForegroundColor Yellow
$Collection | New-CEFMessage -DeviceVendor 'JaredP' -DeviceProduct 'MyScript' -DeviceVersion '2.0' -DeviceEventClassId 'Detection' -Severity 8 | Write-Host -ForegroundColor Red
